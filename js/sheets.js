...
const SheetsAPI = (() => {
  ...
  async function appendPortfolio(row) {
    ...
    const fFormula = `=IF(ISBLANK(B${nextRow}), 0, IF(REGEXMATCH(TO_TEXT(B${nextRow}), "^[0-9]{6}$"), INT(GOOGLEFINANCE("KRX:"&TEXT(B${nextRow},"000000"))), IF(OR(C${nextRow}="나스닥", C${nextRow}="NYSE"), INT(GOOGLEFINANCE(B${nextRow}) * GOOGLEFINANCE("USDKRW")), INT(GOOGLEFINANCE(B${nextRow})))))`;
  ...
  async function updatePortfolio(rowIndex, row) {
    ...
    const fFormula = `=IF(ISBLANK(B${rowIndex}), 0, IF(REGEXMATCH(TO_TEXT(B${rowIndex}), "^[0-9]{6}$"), INT(GOOGLEFINANCE("KRX:"&TEXT(B${rowIndex},"000000"))), IF(OR(C${rowIndex}="나스닥", C${rowIndex}="NYSE"), INT(GOOGLEFINANCE(B${rowIndex}) * GOOGLEFINANCE("USDKRW")), INT(GOOGLEFINANCE(B${rowIndex})))))`;
