...
`kis-proxy.js`에 엔드포인트가 추가되면(예: `/price` 관심 주식 시세, `/reserved-orders` 예약 주문),
위 4~6단계처럼 **편집기 내용을 새 파일 내용으로 교체 → Deploy** 만 다시 하면 됩니다(주소는 그대로).
재배포 전에는 새 기능이 404로 떨어지며 앱은 ‘-’/안내로 표시(나머지 기능엔 영향 없음).
